...
export class CreateTask1760706000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE TYPE "public"."tasks_label_enum" AS ENUM('meeting', 'personal', 'preparation', 'grading')`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE INDEX "IDX_tasks_tutorId" ON "tasks" ("tutorId")`,
        );
        await queryRunner.query(
            `ALTER TABLE "tasks" ADD CONSTRAINT "FK_tasks_tutorId" FOREIGN KEY ("tutorId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
        );
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `ALTER TABLE "tasks" DROP CONSTRAINT "FK_tasks_tutorId"`,
        );
